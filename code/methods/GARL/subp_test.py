            ...
            while True:
                if not shared_ifdone.value:
                    ...
                    loc_ugv_network.load_state_dict(
                        torch.load(os.path.join(log_root_path, 'avg_good_ugv_model.pth'), map_location=torch.device('cpu')))
                    loc_uav_network.load_state_dict(
                        torch.load(os.path.join(log_root_path, 'avg_good_uav_model.pth'), map_location=torch.device('cpu')))
                    ################################## interact with env ####################################
                    # reset env
                    env.reset()
                    obs_generator.reset()
                    for step_id in range(env_conf['max_step_num']):
                        ...
                        for UGV_UAVs_Group_id in range(env_conf['UGV_UAVs_Group_num']):
                            ...
                            if use_loc_ugv_network_flag:
                                ...
                                actions4UGV_UAVs_Group_list[UGV_UAVs_Group_id]['UGV'] = np.array(action_s[0],
                                                                                                 dtype=np.float32)
                                ...
                                sub_rollout_manager.sub_rollout_dict['UGV'][ugv_sub_rollout_id].append_rollout_element(
                                    'obs_neighbor_stopids_vector_s',
                                    step_UGV_input_dict[ugv_id]['obs_neighbor_stopids_vector'])
                                sub_rollout_manager.sub_rollout_dict['UGV'][ugv_sub_rollout_id].append_rollout_element(
